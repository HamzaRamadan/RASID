...
const pointGlowPlugin = {
  id: "pointGlow",
  afterDatasetsDraw(chart: { ctx: any; getDatasetMeta: (arg0: number) => any; config: { type: string; }; }) {
    const ctx = chart.ctx;
    const meta = chart.getDatasetMeta(0);
...
export default function DashboardCharts() {
  ...
  const barData = {
    ...
    datasets: [
      ...
      {
        label: "Remaining",
        data: [10, 9, 10, 9, 8, 9, 9, 8, 9, 7].map((v) => maxBarValue - v),
        backgroundColor: "#555555",
        borderRadius: 6,
    ...
    <div className="w-full min-h-screen bg-[#0A1A23] text-white p-4 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* <div className="bg-[#12323D] p-4 rounded-xl shadow-xl h-[350px] flex flex-col">
          ...
          <div className="flex-1 h-full min-h-[300px]">
            <Line data={lineData} options={lineChartOptions} />
                      <img src="/images/Frame 1.svg" alt="logo" className="w-32 sm:w-32" />
          </div>
